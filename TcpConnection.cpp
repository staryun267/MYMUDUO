...
{
    //给channel注册相应的回调函数，poller通知channel感兴趣的事件发生了，channel执行对应的操作函数
...
void TcpConnection::sendInLoop(const void* data,size_t len)
{
    ...
    /* 若这一次write没有把数据全部发送完，剩余的数据需要保存在缓冲区当中，然后给channel注册EPOLLOUT事件
       Poller发现tcp的发送缓冲区有空间，会通知相应的channel，调用writeCallback_回调方法，也就是调用，
       TcpConnection::handleWrite回调方法，把发送缓冲区的数据全部发送完成
    */
...
void TcpConnection::handleError()
{
    ...
    int err = 0;
    if(::setsockopt(channel_->fd(),SOL_SOCKET,SO_ERROR,&optval,optlen)<0)
    {
        err = errno;
    }
    else

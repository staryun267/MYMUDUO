...
TcpServer::TcpServer(EventLoop *loop,
    ...
    : loop_(CheckLoopNotNull(loop)),
      ...
      started_(0)
{
    //当有新用户连接时会执行TcpServer::newConenction回调
...
TcpServer::~TcpServer()
{
    ...
    {
        TcpConnectionPtr conn(item.second); //这个局部的shared_ptr智能指针对象，出右括号可以自动地释放new出来的TcpConnection对象资源
        item.second.reset();
...
void TcpServer::newConnection(int sockfd,const InetAddress& peerAddr)
{
    ...
    //下面的回调是用户设置给TcpServer->TcpConnection->channel->Poller->channel触发回调
    ...
    ioLoop->runInLoop(
        std::bind(&TcpConnection::connectedEstablished,conn)
    );
}
